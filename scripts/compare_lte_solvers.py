...
Usage:
    ...
    JAX_PLATFORMS=cpu VAJAX_NO_PROGRESS=1 uv run python scripts/compare_lte_solvers.py \
        --benchmark ring --output /tmp/lte_trace_umfpack.json

    # Run on GPU (cuDSS) and save trace:
    JAX_PLATFORMS=cuda VAJAX_NO_PROGRESS=1 uv run python scripts/compare_lte_solvers.py \
