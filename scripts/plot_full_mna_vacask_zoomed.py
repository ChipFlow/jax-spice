#!/usr/bin/env python3
"""Plot Full MNA vs VACASK - zoomed 7-11ns window matching original format."""
...
def main():
    ...
    # Create plot - 3 panels, zoomed window
    fig, axes = plt.subplots(3, 1, figsize=(10, 9), sharex=True)

    # Zoomed window matching original reference (7-11ns)
    t_start, t_end = 7e-9, 11e-9
    ...
    ax1.plot(t_vacask[mask_v] * 1e9, V1_vacask[mask_v], 'b-', lw=1.5, label='VACASK V(1)', alpha=0.9)
    ax1.plot(t_vacask[mask_v] * 1e9, V2_vacask[mask_v], 'b--', lw=1.5, label='VACASK V(2)', alpha=0.9)
    ax1.plot(t_mna[mask_m] * 1e9, V1_mna[mask_m], 'r-', lw=1.5, label='Full MNA V(1)', alpha=0.9)
    ax1.plot(t_mna[mask_m] * 1e9, V2_mna[mask_m], 'r--', lw=1.5, label='Full MNA V(2)', alpha=0.9)
    ax1.set_ylabel('Voltage [V]', fontsize=11)
    ax1.set_ylim(-0.2, 1.4)
    ax1.legend(loc='upper right', ncol=2, fontsize=9)
    ax1.grid(True, alpha=0.3)
    ax1.set_title('Ring Oscillator: Full MNA vs VACASK (7-11ns zoom)', fontsize=12, fontweight='bold')

    # Panel 2: Current in µA
    ax2 = axes[1]
    ax2.plot(t_vacask[mask_v] * 1e9, I_vacask[mask_v] * 1e6, 'b-', lw=1.5, label='VACASK', alpha=0.9)
    ax2.plot(t_mna[mask_m] * 1e9, I_mna[mask_m] * 1e6, 'r-', lw=1.5, label='Full MNA', alpha=0.9)
    ...
    ax3.plot(t_didt_vacask[mask_dv] * 1e9, dIdt_vacask[mask_dv] * 1e-6, 'b-', lw=1.2, label='VACASK', alpha=0.9)
    ax3.plot(t_didt_mna[mask_dm] * 1e9, dIdt_mna[mask_dm] * 1e-6, 'r-', lw=1.2, label='Full MNA', alpha=0.9)
    ...
    output_file = 'ring_full_mna_vacask_zoomed.png'
    plt.savefig(output_file, dpi=150, bbox_inches='tight')
    print(f"Saved: {output_file}")

    # Print metrics
    print("\n" + "=" * 60)
    print("Comparison Metrics (7-11ns zoomed window):")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("SUMMARY: Full MNA vs old high-G method")
    print("=" * 60)
    print("Old high-G method: +45% sharper dI/dt than VACASK")
    print(f"New Full MNA:      {didt_diff:+.1f}% (smoother, more accurate)")
    print("Full MNA extracts current as primary unknown - no G=1e12 noise")
