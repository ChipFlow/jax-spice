...
class TestSolutionHistory:
    ...
    def test_push_and_depth(self):
        ...
        # n_total=1 for V, n_unknowns=1 for Q (same size for this simple test)
        history = SolutionHistory.create(n_total=1, n_unknowns=1, max_depth=3)
        assert history.depth == 0

        # Push first solution (immutable API returns new history)
        V1 = jnp.array([1.0])
        Q1 = jnp.array([1e-12])
        history = history.push(V1, Q1, dt_new=1e-9)
        assert history.depth == 1

        # Push second
        V2 = jnp.array([2.0])
        Q2 = jnp.array([2e-12])
        history = history.push(V2, Q2, dt_new=1e-9)
        assert history.depth == 2

        # Most recent should be at get_V(0), second most recent at get_V(1)
        np.testing.assert_allclose(history.get_V(0), V2)
        np.testing.assert_allclose(history.get_V(1), V1)

    def test_max_depth_eviction(self):
        """Test that old solutions are evicted when max_depth is reached."""
        from jax_spice.analysis.transient.adaptive import SolutionHistory

        history = SolutionHistory.create(n_total=1, n_unknowns=1, max_depth=2)

        for i in range(5):
            V = jnp.array([float(i)])
            Q = jnp.array([float(i) * 1e-12])
            history = history.push(V, Q, dt_new=1e-9)

        # Should only have last 2 (circular buffer)
        assert history.depth == 2
        np.testing.assert_allclose(history.get_V(0), jnp.array([4.0]))
        np.testing.assert_allclose(history.get_V(1), jnp.array([3.0]))

    def test_create_fresh(self):
        """Test creating a fresh history (replaces old clear() behavior)."""
        from jax_spice.analysis.transient.adaptive import SolutionHistory

        history = SolutionHistory.create(n_total=1, n_unknowns=1, max_depth=4)
        history = history.push(jnp.array([1.0]), jnp.array([1e-12]), 1e-9)
        history = history.push(jnp.array([2.0]), jnp.array([2e-12]), 1e-9)
        assert history.depth == 2

        # Create fresh history (immutable design - no clear() needed)
        history = SolutionHistory.create(n_total=1, n_unknowns=1, max_depth=4)
        assert history.depth == 0
